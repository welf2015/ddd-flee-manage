...
export function OnboardingDetailSheet({ onboardingId, open, onClose }: OnboardingDetailSheetProps) {
  const {
    ...
    mutate,
    isLoading,
    error, // destructure error to handle it
  } = useSWR(open && onboardingId ? `onboarding-${onboardingId}` : null, () => fetcher(onboardingId))

  if (error) {
    console.log("[v0] Onboarding fetch error:", error)
  }
  if (onboarding) {
    console.log("[v0] Onboarding data:", onboarding)
  }
          ...
          <>
            <SheetHeader className="mb-6">
              <div className="flex items-start justify-between">
                ...
                <Badge variant={onboarding.status === "Completed" ? "default" : "secondary"}>{onboarding.status}</Badge>
              </div>
            </SheetHeader>
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1">
                              ...
                              {item.checklist_item?.description && (
                                <p className="text-xs text-muted-foreground mt-1">{item.checklist_item.description}</p>
                              )}

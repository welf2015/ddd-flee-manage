...
export function generateIncidentReportEmail({
  incidentNumber,
  severity,
  vehicle,
  location,
  ...
  return {
    ...
    html: renderEmailLayout({
      ...
      badge: { label: severity || "Severity: N/A", tone: severity === "High" ? "danger" : severity === "Medium" ? "warning" : "info" },
      rows: [
...
export function generateTopupEmail({
  accountName,
  vendorType,
  amount,
  depositedBy,
